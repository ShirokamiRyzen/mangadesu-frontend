...
export default function Beranda() {
    ...
    function getdata(pagination = page, reset = false) {
        ...
        time = setTimeout(async () => {
            const data = await axios.get(`https://mangadesu-backend.vercel.app/manga?page=${reset ? 1 : pagination}&genre=${cat}&order=${query}&title=${search}`);
    ...
    async function getGenres() {
        const response = await axios.get("https://mangadesu-backend.vercel.app/genre");
        setGenres(response.data);
    }

    async function getSearch() {
        const response = await axios.get(`https://mangadesu-backend.vercel.app/search?query=${search}`);
    ...
    async function getManga() {
        try {
            const data = await axios.get(`https://mangadesu-backend.vercel.app/manga/${slug}`);
            setManga(data.data);
        } catch (e) {
            return mode404();
        }
    ...
    async function getdataChapters() {
        ...
        loadingPenuh(true)
        const data = await axios.get(`https://mangadesu-backend.vercel.app/manga/${slug}/${indexChapter}`);
                                ...
                                {chapter.map((image, index) =>
                                    <div className="col-md-12 p-0 d-flex justify-content-center" key={index}>
                                        {!image.gambar.includes("Chapter") ?
                                            <img src={`https://mangadesu-backend.vercel.app/gambar?url=${image.gambar}`} className="img-fluid" alt={index} />
                                            :
                                            <h1>{image.gambar}</h1>
                                        }
                                    </div>
